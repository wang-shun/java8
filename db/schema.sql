...
CREATE TABLE `doctor_groups` (
  ...
  `farm_id` bigint(20) DEFAULT NULL COMMENT '猪场id',
  `farm_name` varchar(64) DEFAULT NULL COMMENT '猪场名称',
  `group_code` varchar(640) DEFAULT NULL COMMENT '猪群号',
...
-- 消息表 2016-06-07
DROP TABLE IF EXISTS `doctor_message_rule_template_tracks`;
CREATE TABLE IF NOT EXISTS `doctor_message_rule_template_tracks` (
	`id`	BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
	...
	PRIMARY KEY(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='猪场软件消息规则模板表(历史跟踪)';
...
CREATE TABLE `doctor_material_price_in_ware_houses` (
  ...
  `remainder` bigint(20) NOT NULL COMMENT '本次入库量的剩余量，比如本次入库100个，那么就是这100个的剩余量，减少到0时删除',
