...
public class DoctorMaterialInfos {
    ...
    public Boolean realProduceMaterial(@RequestParam("farmId") Long farmId,
        ...
        DoctorWareHouseBasicDto doctorWareHouseBasicDto;
    ...
    public void buildProduceInfo(DoctorMaterialInfo.MaterialProduce materialProduce){
        try{
            materialProduce.getMaterialProduceEntries().forEach(
                    s-> s.setMaterialName(RespHelper.or500(doctorBasicMaterialReadService.findBasicMaterialById(s.getMaterialId())).getName())
            );

            materialProduce.getMedicalProduceEntries().forEach(
                    s-> s.setMaterialName(RespHelper.or500(doctorBasicMaterialReadService.findBasicMaterialById(s.getMaterialId())).getName())
            );
        }catch (Exception e){
