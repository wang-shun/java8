...
public class DoctorRangeReportWriteServiceImpl implements DoctorRangeReportWriteService {
    ...
    private static final JsonMapperUtil FROM_JSON = JsonMapperUtil.JSON_NON_DEFAULT_MAPPER;
    ...
    public Response<Boolean> generateDoctorRangeReports(List<Long> farmIds, Date date) {
        try{
            ...
            farmIds.stream().parallel().forEach(farmId -> updateActualIndicator(farmId, date));
    ...
    private void add(List<DoctorFarmEarlyEventAtDto> list3,
        ...
        if (notNull(json)) {
            if (Objects.equals(type, DoctorEventModifyRequest.TYPE.PIG.getValue())) {
                DoctorPigEvent event = FROM_JSON.fromJson(json, DoctorPigEvent.class);
                list3.add(new DoctorFarmEarlyEventAtDto(event.getFarmId(), event.getEventAt()));
            } else {
                DoctorGroupEvent event = FROM_JSON.fromJson(json, DoctorGroupEvent.class);
                list3.add(new DoctorFarmEarlyEventAtDto(event.getFarmId(), event.getEventAt()));
            }
        }
    }
